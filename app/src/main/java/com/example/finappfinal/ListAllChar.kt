...
class ListAllChar : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_list_all_char)
    ...
    private fun requestGetAllChar(){
        lifecycleScope.launch() {
            try {
                progressBar.visibility = View.VISIBLE
                val response = withContext(Dispatchers.IO) {
                    harryPotterAPI?.getAllChar()
                }
                progressBar.visibility = View.GONE

                if (!response.isNullOrEmpty()) {
                    characterAdapter = CharacterAdapter(response, this@ListAllChar){}
